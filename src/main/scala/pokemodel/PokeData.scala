...
object PokeData {
  ...
  private def processPokemonName(name: String): String =
  ...
  private var name2ID: Map[String, Int] = Map()
  ...
  for (line <- Source.fromFile(baseStatsFile).getLines
    ...
    val data = line.split(", ")
    val Array(idS, nameS, hpS, attackS, defenseS, speedS, specialS, type1S, type2S) = data
    val Array(id, hp, attack, defense, speed, special) =
      Array(idS, hpS, attackS, defenseS, speedS, specialS).map(_.toInt)
    val name = processPokemonName(nameS)  // store lowercase names
  ...
  assert(name2ID.size == PokemonBuilder.numPokemon)
  ...
  def nameToID(name: String): Int = {
    // accepts any kind of capitalization, spacing, and hyphens
    val formatted = processPokemonName(name)
    name2ID(formatted)
  }
}
